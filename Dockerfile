...
# -----------------
FROM --platform=linux/amd64 nvidia/cuda:12.3.2-cudnn9-devel-ubi8 as amd64ubi8
# Install compiler for .compile() with PyTorch 2.0 and nano for devcontainers
RUN yum install -y git gcc gcc-c++ nano glibc && yum clean all
...
# -----------------
FROM --platform=linux/amd64 nvidia/cuda:12.3.2-cudnn9-devel-ubuntu22.04 as amd64ubuntu
# Install compiler for .compile() with PyTorch 2.0 and nano for devcontainers
RUN apt-get update && apt-get install -y git gcc g++ nano openssh-client parallel curl && apt-get clean

ENV TZ=US \
    DEBIAN_FRONTEND=noninteractive

# Remove fore release
RUN apt-get update && apt-get install -y texlive texlive-xetex texlive-lang-german texlive-latex-extra cm-super dvipng && apt-get clean
# Copy lockfile to container
COPY conda-lock.yml /locks/conda-lock.yml
...
# -----------------
FROM --platform=linux/ppc64le nvidia/cuda:12.3.2-cudnn9-devel-ubi8 as ppc64leubi8
# Install compiler for .compile() with PyTorch 2.0
RUN yum install -y gcc gcc-c++ parallel && yum clean all
...
#RUN CMAKE_ARGS="-DGGML_BLAS=ON -DGGML_BLAS_VENDOR=OpenBLAS" pip install llama-cpp-python==0.3.1 --upgrade --force-reinstall --no-cache-dir
