...
find "$search_dir" -type f -path "*/output/*" -name "*_unstructured*.pkl" | while read -r output_file; do
    # Extract the 'unstructured_X_X .pkl' part and construct input path
    base_name=$(basename "$output_file" | sed -E 's/.*_(unstructured_[0-9]+_[0-9]+\.pkl)$/\1/')
    ...
    python $PYTHON_SCRIPT --input_path "$input_file" --output_path "$output_file" --num_gpus 1
