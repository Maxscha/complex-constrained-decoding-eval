...
JOBS=(
    ...
    "mistral structured 12"
    "mistral-instruct structured 12"
    "phi3 structured 12"
    "olmo structured 12"
    "olmo-instruct structured 12"
    "llama31-instruct structured 7"
    "llama31-base structured 4"
    "deepseek structured 12"
    "deepseek-chat structured 2"
    "deepseek-coder structured 12"
    "deepseek-coder-instruct structured 2"
)
...
for triplet in "${JOBS[@]}"; do
    ...
    if [ $TYPE == "structured" ]; then
        commands+=("src/generate.py --input_files $INPUT_FILE --output_files $OUTPUT_FILE --model $MODEL --use_logits_processor True --schema_type thinking")
    else
        # commands+=("src/generate.py --input_files $INPUT_FILE --output_files $OUTPUT_FILE --model $MODEL --use_logits_processor False")
