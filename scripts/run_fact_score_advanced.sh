...
FOLDER=${1:-"data/fact_score_advanced_cleaned"}
    ...
    for PROMPT in ${PROMPTS[@]}; do
        for TYPE in ${TYPES[@]}; do
            ...
            if [ ! -f $INPUT_FILE ]; then
                python3 src/prepare_data_factscore_advanced.py \
                    --output $INPUT_FILE \
                    --type $TYPE \
                    --random_seed $SEED
            fi
...
for triplet in "${JOBS[@]}"; do
    ...
    if [ $TYPE == "structured" ]; then
        commands+=("src/generate.py --input_files $INPUT_FILE --output_files $OUTPUT_FILE --model $MODEL --use_logits_processor True --max_tokens 2048 --schema_type factscore")
    else
        commands+=("src/generate.py --input_files $INPUT_FILE --output_files $OUTPUT_FILE --model $MODEL --use_logits_processor False --max_tokens 2048 --schema_type factscore")
    fi
done
